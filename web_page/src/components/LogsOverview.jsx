            ...
            {!isLoading &&
                < div className="flex flex-col gap-8 h-1/5 w-full items-start" >
                    < div className="grid grid-cols-2 md:grid-cols-4 gap-8 h-full w-full justify-between items-start" >
                        <div className={`card bg-neutral  card-border shadow-accent  hover:cursor-pointer hover:bg-neutral-500`} onClick={() => router.push(`/main_navigation/logs?${urlDeviceFilter}`)}>
                            <div className="card-body">
                                <h1 className="card-title justify-start text-neutral-content">Logs: {logCount}</h1>
                            </div>
                        </div>

                        <div className="card bg-base-300 card-border shadow-accent  hover:cursor-pointer hover:bg-neutral-500" onClick={() => router.push(`/main_navigation/logs?type=SISTEMA&${urlDeviceFilter}`)}>
                            <div className="card-body">
                                <h1 className="card-title justify-start ">Sistema: {systemCount}</h1>
                            </div>
                        </div>
                        <div className="card bg-warning card-border shadow-accent  hover:cursor-pointer hover:bg-neutral-500" onClick={() => router.push((`/main_navigation/logs?type=BATERIA&${urlDeviceFilter}`))}>
                            <div className="card-body">
                                <h1 className="card-title justify-start ">Aviso Batería: {batteryWarningCount}</h1>
                            </div>
                        </div>
                        <div className="card bg-error card-border shadow-accent  hover:cursor-pointer hover:bg-neutral-500" onClick={() => router.push(`/main_navigation/logs?type=DETECCION&${urlDeviceFilter}`)}>
                            <div className="card-body">
